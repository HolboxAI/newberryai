...
def coder_CLI():
    """
    Command-line interface for the document summarizer AI Assistant
    """
    parser = argparse.ArgumentParser(description="AI document summarizer Assistant")

    parser.add_argument("--code", "-c", type=str, 
                        help="Provide the document path for summarizing")
    ...
    if args.gradio:
        print("Launching web interface for document summarizer AI Assistant")
        summarizer.start_gradio()
    elif args.interactive:
        print("Starting interactive document summarizer session")
        summarizer.run_cli()
    elif args.code:
        print(f"Document Path: {args.code}\n")
        response = summarizer.ask(args.code)
        print("Summary:")
        print(response)
    else:
        parser.print_help()
