...
class Bill_extractor:
    ...
    def __init__(self):
        ...
        self.assistant = ChatQA(system_prompt=Sys_Prompt)
...
def Bill_exc_CLI():
    """Command-line interface for the Bill Extractor tool"""
    parser = argparse.ArgumentParser(description="AI Document Analysis Tool")

    parser.add_argument("--image", "-img", type=str, help="Path to a document image to analyze")
    ...
    bill_exc_chat = Bill_extractor()
    
    if args.gradio:
        print(f"Launching Gradio interface for Document Analysis")
        bill_exc_chat.start_gradio()

    elif args.interactive:
        bill_exc_chat.run_cli()

    elif args.image:
        # Validate that the image file exists
        if not os.path.exists(args.image):
            print(f"Error: Document file not found at path: {args.image}")
            sys.exit(1)
        
        print(f"Analyzing document: {args.image}")
        response = bill_exc_chat.analyze_document(args.image)
